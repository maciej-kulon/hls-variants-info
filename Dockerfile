...
RUN \
  ...
  cp -r /tmp/vmaf/model/* /usr/local/share/model/ && \
  rm -rf ${DIR};


## libzimg https://github.com/sekrit-twc/zimg/archive/refs/tags/release-3.0.4.tar.gz
ENV LIBZIMG_VERSION=3.0.4
RUN \
  DIR=/tmp/zscale && \
  mkdir -p ${DIR} && \
  cd ${DIR} && \
  curl -sLO https://github.com/sekrit-twc/zimg/archive/refs/tags/release-${LIBZIMG_VERSION}.tar.gz && \
  tar -xz --strip-components=1 -f release-${LIBZIMG_VERSION}.tar.gz && \
